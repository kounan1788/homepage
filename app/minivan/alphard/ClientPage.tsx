...
export default function Page() {
    const basePrice = carBasePrices['/minivan/alphard'];

    const colors: ColorOption[] = useMemo(() => [
        { name: 'ブラック', price: 0, colorCode: '#1a1a1a' },
        { name: 'プラチナホワイトパールマイカ', price: 750, colorCode: '#f8f8f8' },
    ], []);

    const options: VehicleOption[] = useMemo(() => [
        { name: 'デジタルインナーミラー', price: 0, isDefault: true, category: 'セーフティ' },
        { name: '左右独立ムーンルーフ', price: 0, isDefault: true, category: 'エクステリア' },
        { name: '純正フロアマット', price: 0, isDefault: true, category: 'インテリア' },
        { name: 'カラーヘッドアップDP', price: 0, isDefault: true, category: 'ナビ・電装' },
        { name: 'ユニバーサルステップ', price: 2400, category: 'エクステリア' },
        { name: 'トヨタチームメイト', price: 0, isDefault: true, category: 'セーフティ' },
        { name: '13.2型有機EL後席DP+CD/DVD', price: 5800, category: 'ナビ・電装' },
    ...
    const contactUrl = (() => {
        ...
        return buildContactUrl({
            category: 'ノレタ',
            lines: [
                '車種: トヨタ アルファード（Z GAS・2WD・2,500cc）',
                ...
                <div className="max-w-5xl mx-auto">
                    ...
                    <nav aria-label="パンくずリスト" className="mb-3">
                        <ol className="flex flex-wrap items-center text-xs text-slate-400">
                            ...
                            <li className="mx-2">›</li>
                            <li className="text-slate-200">トヨタ アルファード</li>
                        </ol>
            ...
            <div className="max-w-5xl mx-auto px-4 md:px-8 py-8 pb-32">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                                ...
                                {colors.map((color) => (
                                    ...
                                    >
                                        <div className={`relative size-16 md:size-20 rounded-full transition-ui duration-200 ${selectedColor === color.name
                                            ...
                                            ></div>
                                            {(color.colorCode === '#FFFFFF' || color.colorCode === '#f8f8f8') && (
                        ...
                        {/* ボーナス払いシミュレーター */}
                        <BonusPaymentSimulator principal={carLoanPrincipals['/minivan/alphard']} />
                    </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur border-t border-gray-200 z-40 pb-[env(safe-area-inset-bottom)] p-4 pb-[calc(1rem+env(safe-area-inset-bottom))]">
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    <div className="hidden sm:block">
                        <p className="font-bold text-slate-800">トヨタ アルファード</p>
                        <p className="text-xs text-slate-500">Z GAS・2WD・2,500cc</p>
                    </div>
